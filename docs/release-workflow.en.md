...
- **Before opening the Pull Request**, generate a PR description using Git Friday:
  ...
  friday run -p pr
  ```
  This will use the `pr` profile to generate an AI-powered summary of all changes in the PR, which you can use for the PR description.
...
5.  **Create a release on GitHub:**
    - First, generate the changelog using Git Friday:
      ```bash
      friday run -p changelog
      ```
      This will use the `changelog` profile to generate release notes based on all changes since the last release.
